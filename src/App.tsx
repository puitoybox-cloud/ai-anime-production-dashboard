...
type ViewMode = 'work-list' | 'episode-list' | 'scene-list' | 'scene-edit' | 'asset-library';
...
type ExportScope = 'all' | 'work' | 'asset-library' | 'episode' | 'scene' | 'production-step' | 'selected-assets';
...
const SCHEMA_VERSION = '0.6' as const;
...
const dummyScenes: Array<Omit<Scene, 'thumbnail' | 'tags' | 'favorite' | 'productionMemo' | 'productionSteps' | 'assets' | 'assetIds' | 'createdAt' | 'updatedAt'>> = [
...
function createProductionSteps(status: ProductionStatus = '未着手'): ProductionStep[] { const timestamp = nowIso(); return productionStepDefinitions.map((step) => ({ ...step, status, memo: '', relatedAssets: '', relatedAssetIds: [], relatedPrompts: '', updatedAt: timestamp })); }
...
function migrateAsset(raw: Partial<Asset> & { id?: string | number; name?: string }, index: number): Asset {
  ...
  return {
    ...
    creationMethod: isCreationMethod(raw.creationMethod) ? raw.creationMethod : 'AI生成', software: typeof raw.software === 'string' ? raw.software : '', creationAi: typeof raw.creationAi === 'string' ? raw.creationAi : '', sourceAssetId: typeof raw.sourceAssetId === 'string' ? raw.sourceAssetId : '', versionName: typeof raw.versionName === 'string' ? raw.versionName : '', japanesePrompt: typeof raw.japanesePrompt === 'string' ? raw.japanesePrompt : '', englishPrompt: typeof raw.englishPrompt === 'string' ? raw.englishPrompt : '', negativePrompt: typeof raw.negativePrompt === 'string' ? raw.negativePrompt : '', consistencyMemo: typeof raw.consistencyMemo === 'string' ? raw.consistencyMemo : '', character, createdAt, updatedAt: typeof raw.updatedAt === 'string' ? raw.updatedAt : createdAt,
  };
}
...
function loadInitialData(): DashboardData {
  ...
  if (savedV03) { try { const migrated = normalizeDashboard(JSON.parse(savedV03)); if (migrated) { localStorage.setItem(STORAGE_KEY_V06, JSON.stringify(migrated)); return migrated; } return createDefaultDashboardData(); } catch { return createDefaultDashboardData(); } }
  ...
  if (savedV02) {
    try {
      ...
      if (migrated) { localStorage.setItem(STORAGE_KEY_V06, JSON.stringify(migrated)); return migrated; }
...
export function App() {
  ...
  useEffect(() => { localStorage.setItem(STORAGE_KEY_V06, JSON.stringify(dashboard)); }, [dashboard]);
  ...
  const buildExportData = (kind: ExportKind, scope: ExportScope) => {
    ...
    const rawData = scope === 'all' ? dashboard : scope === 'work' ? selectedWork : scope === 'asset-library' ? selectedWork?.assetLibrary : scope === 'episode' ? selectedEpisode : scope === 'scene' ? selectedScene : scope === 'production-step' ? selectedScene?.productionSteps : selectedWork?.assetLibrary.filter((asset) => selectedExportAssetIds.includes(asset.id));
  ...
  const exportJson = () => { const data = buildExportData(exportKind, exportScope); downloadJson(data, `dream-architect-studio-${exportKind}-${exportScope}-v0.6.json`); };
  ...
  return <main className="app">
    <header className="app-header"><div><p className="eyebrow">Dream Architect Studio</p><h1>AIアニメ制作ダッシュボード v0.6</h1></div><button onClick={() => setJsonPanelOpen((open) => !open)}>JSON連携</button></header>
    ...
    {viewMode === 'episode-list' && selectedWork && <EpisodeList work={selectedWork} onOpenAssetLibrary={() => setViewMode('asset-library')} onAdd={addEpisode} onOpen={openEpisode} onDelete={deleteEpisode} onMove={moveEpisode} onUpdate={updateSelectedWork} />}
    ...
    {viewMode === 'scene-edit' && selectedScene && <SceneEditor scene={selectedScene} libraryAssets={selectedWork?.assetLibrary ?? []} onUpdate={updateSelectedScene} onDelete={() => deleteScene(selectedScene.id)} onAddAsset={addAsset} onUpdateAsset={updateAsset} onDeleteAsset={deleteAsset} />}
  </main>;
}
...
function SceneEditor({ scene, libraryAssets, onUpdate, onDelete, onAddAsset, onUpdateAsset, onDeleteAsset }: { scene: Scene; libraryAssets: Asset[]; onUpdate: <K extends keyof Scene>(key: K, value: Scene[K]) => void; onDelete: () => void; onAddAsset: () => void; onUpdateAsset: (assetId: string, patch: Partial<Asset>) => void; onDeleteAsset: (assetId: string) => void }) {
  ...
  return <>
    ...
    <ProductionStepPanel assets={libraryAssets} steps={scene.productionSteps} selectedStep={selectedStep} onSelect={setSelectedStepId} onUpdate={updateStep} />
    <AssetList assets={scene.assets} onAdd={onAddAsset} onUpdate={onUpdateAsset} onDelete={onDeleteAsset} />
  </>;
}

function ProductionStepPanel({ assets, steps, selectedStep, onSelect, onUpdate }: { assets: Asset[]; steps: ProductionStep[]; selectedStep?: ProductionStep; onSelect: (id: ProductionStepId) => void; onUpdate: (patch: Partial<ProductionStep>) => void }) {
  return <section className="panel step-panel"><div className="section-heading"><div><h2>制作工程管理</h2><p className="muted">8工程の状態から場面の進捗率を自動計算します。</p></div></div><div className="step-grid">{steps.map((step, index) => <button key={step.id} type="button" className={`step-card ${selectedStep?.id === step.id ? 'active' : ''}`} onClick={() => onSelect(step.id)}><span className="step-number">{index + 1}</span><strong>{step.name}</strong><StatusBadge status={step.status} /><small>素材：{step.relatedAssets ? 'あり' : '未設定'} / プロンプト：{step.relatedPrompts ? 'あり' : '未設定'}</small></button>)}</div>{selectedStep && <div className="step-editor"><h3>{selectedStep.name}を編集</h3><label className="field"><span>状態</span><select value={selectedStep.status} onChange={(event) => onUpdate({ status: event.target.value as ProductionStatus })}><option>未着手</option><option>作業中</option><option>確認中</option><option>完了</option></select></label><TextArea label="メモ" value={selectedStep.memo} onChange={(memo) => onUpdate({ memo })} /><TextArea label="関連素材（自由入力）" value={selectedStep.relatedAssets} onChange={(relatedAssets) => onUpdate({ relatedAssets })} /><AssetSelector assets={assets} selectedIds={selectedStep.relatedAssetIds} onChange={(relatedAssetIds) => onUpdate({ relatedAssetIds })} title="関連アセット（ライブラリから選択）" /><TextArea label="関連プロンプト" value={selectedStep.relatedPrompts} onChange={(relatedPrompts) => onUpdate({ relatedPrompts })} /><p className="thumbnail-info">工程更新：{formatDate(selectedStep.updatedAt)}</p></div>}</section>;
}
